...
#     log_filename=${experiment_name}/${hydra_scaled_inits[i]}.log
#     python ./experiments/prompt_sparsity/lm_vp_prune.py --experiment_name ${experiment_name} --prune_method ${prune_methods[0]} --label_mapping_mode ${label_mapping_modes[0]} --prompt_method ${prompt_methods[0]} --hydra_scaled_init ${hydra_scaled_inits[i]} --gpu ${gpus[i]}  --input_size ${input_sizes[0]} --pad_size ${pad_sizes[0]} --mask_size ${mask_sizes[0]} --pruning_times ${pruning_times} --epochs ${epochs} > $log_filename 2>&1 &
# done
