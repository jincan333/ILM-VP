...
experiment_name='ablation_method_vp_search'
            ...
            for l in ${!prune_methods[@]};do
                for m in ${!seeds[@]};do
                    for n in ${!score_vp_ratios[@]};do
                        for o in ${!input_sizes[@]};do
                            for p in ${!pad_sizes[@]};do
                                log_filename=${foler_name}/${networks[j]}_${datasets[i]}_${prune_modes[k]}_${prune_methods[l]}_${seeds[m]}_${input_sizes[o]}_${pad_sizes[p]}_ratio${score_vp_ratios[n]}_${ff_optimizer}_${ff_lr}_${vp_optimizer}_${vp_lr}_${hydra_optimizer}_${hydra_lr}.log
                                    python ./core/vpns_explore_method.py \
                                        ...
                                        --input_size ${input_sizes[o]} \
                                        --pad_size ${pad_sizes[p]} \
                                        --gpu ${gpus[p]} \
                                        --epochs ${epochs} \
                                        --seed ${seeds[m]} \
                                        > $log_filename 2>&1 &
