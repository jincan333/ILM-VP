...
def main():    
    ...
    global args
    parser.add_argument('--prune_mode', type=str, default='score+vp_weigths', choices=['score+vp_weight', 'score+weight_vp', 'weight+vp_score', 'score_weight'], help='prune method implement ways')
    parser.add_argument('--prune_method', type=str, default='vpns', choices=['vpns', 'bip', 'random', 'imp', 'omp', 'grasp', 'snip', 'synflow', 'hydra'])
...
def train(train_loader, val_loader, network, epoch, label_mapping, visual_prompt, mask, args, weight_optimizer, vp_optimizer, score_optimizer, weight_scheduler, vp_scheduler, score_scheduler):
    ...
    for i, (train_batch, val_batch) in enumerate(zip(train_loader, val_loader)):
        ...
        if visual_prompt:
            ...
            fx = label_mapping(network(val_x))
        loss = F.cross_entropy(fx, val_y, reduction='mean')
        init_gradients(weight_optimizer, vp_optimizer, score_optimizer)
        loss.backward()
        weight_optimizer.step()
        if args.prune_mode!= 'score_weight' and args.prune_mode == 'weight+vp_score':
            if args.current_steps % args.step_division==0:
                # vp_optimizer.step()
        ...
        if visual_prompt:
            fx = label_mapping(network(visual_prompt(train_x)))
        else:
            fx = label_mapping(network(train_x))
        ...
        if args.prune_mode!= 'score_weight' and args.prune_mode == 'score+weight_vp':
            if args.current_steps % args.step_division==0:
                vp_optimizer.step()
        ...
        if args.prune_mode!= 'score_weight' and args.prune_mode == 'score+vp_weight':
            if args.current_steps % args.step_division==0:
                # vp_optimizer.step()
                update_visual_prompt(visual_prompt, train_x, train_y)
        # reset threshold
