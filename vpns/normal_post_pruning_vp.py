...
def main():    
    ...
    global args
    parser.add_argument('--prune_mode', type=str, default='weight+vp', choices=['weight', 'weight+vp'], help='prune method implement ways')
    ...
    parser.add_argument('--dataset', default="cifar10", choices=['cifar10', 'cifar100', 'flowers102', 'dtd', 'food101', 'oxfordpets', 'stanfordcars', 'tiny_imagenet', 'imagenet'])
    ...
    save_path = os.path.join(args.save_dir, args.experiment_name, args.network, args.dataset, 
                ...
                args.weight_optimizer+'_'+args.weight_vp_optimizer, 
                args.weight_scheduler+'_'+args.weight_vp_scheduler,
                'LR'+str(args.weight_lr)+'_'+str(args.weight_vp_lr),  
    ...
    for epoch in range(args.epochs):
        ...
        if args.prune_method in ('imp', 'omp'):
            train_acc = train(train_loader, val_loader, 'pretrain', network, epoch, label_mapping, visual_prompt, mask, 
    ...
    for state in range(1, len(args.density_list)):
        ...
        print('******************************************')
        for epoch in range(args.epochs):
            train_acc = train(train_loader, val_loader, 'finetune', network, epoch, label_mapping, visual_prompt, mask, 
                            weight_optimizer=weight_optimizer, vp_optimizer=weight_vp_optimizer,
                            weight_scheduler=weight_scheduler, vp_scheduler=weight_vp_scheduler)
            val_acc = evaluate(test_loader, network, label_mapping, visual_prompt)
            all_results['train_acc'].append(train_acc)
        ...
        visual_prompt, score_optimizer, score_scheduler, score_vp_optimizer, score_vp_scheduler, weight_optimizer, weight_scheduler, weight_vp_optimizer, weight_vp_scheduler, checkpoint, best_acc, all_results = init_ckpt_vp_optimizer(network, visual_prompt_init, mapping_sequence, None, args)
        print('******************************************')
        print(f'pruning state {state} prompt')
        print('******************************************')
        for epoch in range(args.epochs):
            train_acc = train(train_loader, val_loader, 'prompt', network, epoch, label_mapping, visual_prompt, mask, 
                            weight_optimizer=weight_optimizer, vp_optimizer=weight_vp_optimizer,
                            weight_scheduler=weight_scheduler, vp_scheduler=weight_vp_scheduler)
            val_acc = evaluate(test_loader, network, label_mapping, visual_prompt)
            all_results['train_acc'].append(train_acc)
...
def train(train_loader, val_loader, phase, network, epoch, label_mapping, visual_prompt, mask, weight_optimizer, vp_optimizer, weight_scheduler, vp_scheduler):
