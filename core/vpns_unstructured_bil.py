...
from unstructured_hydra import set_hydra_prune_rate, set_hydra_network, calculate_threshold


def main():    
    parser = argparse.ArgumentParser(description='PyTorch Visual Prompt + Prune Experiments')
    global args
    parser.add_argument('--prune_mode', type=str, default='score+vp_weigths', choices=['score+vp_weights', 'weights+vp_score', 'score+weights_vp', 'score_weights_vp', 'score_weights'], help='prune method implement ways')
    parser.add_argument('--prune_method', type=str, default='vpns', choices=['vpns', 'bi p', 'random', 'imp', 'omp', 'grasp', 'snip', 'synflow', 'hydra'])
    parser.add_argument('--ckpt_directory', type=str, default='', help='sub-network ckpt directory')
    parser.add_argument('--weights_optimizer', type=str, default='adam', help='The optimizer to use.', choices=['sgd', 'adam'])
    parser.add_argument('--weights_scheduler', default='cosine', help='decreasing strategy.', choices=['cosine', 'multistep'])
    parser.add_argument('--weights_lr', default=0.001, type=float, help='initial learning rate')
    parser.add_argument('--weights_weight_decay', default=1e-4, type=float, help='finetune weight decay')
